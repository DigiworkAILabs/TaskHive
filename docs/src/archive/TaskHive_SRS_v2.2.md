...
### Version 2.2 - Modular Monolithic Architecture
...
│   │   │       │   ├── config/
│   │   │       │   │   ├── SecurityConfig.java                # Spring Security + JWT Cookie filter chain
...
| NFR-SEC-05 | Passwords hashed with BCrypt (strength 12) |
| NFR-SEC-06 | Refresh tokens stored as SHA-256 hash in DB |
| NFR-SEC-07 | Reset and activation tokens stored as SHA-256 hash in DB |
| NFR-SEC-08 | SQL injection prevented via JPA parameterized queries |
| NFR-SEC-09 | File uploads validated for type (JPG/PNG/WebP for photos) and size (max 5MB photos, max 20MB attachments) |
| NFR-SEC-10 | RBAC enforced at API and service layer — no client-side permission checks |
| NFR-SEC-11 | No sensitive data (passwords, tokens, PII) in logs |
| NFR-SEC-12 | GDPR compliance: user data export and deletion capability |
| NFR-SEC-13 | Content Security Policy, X-Frame-Options, HSTS headers enabled |
...
**Decision**: JWT stored in HttpOnly, Secure, SameSite=Strict cookies only. localStorage is XSS-vulnerable. Cookie approach requires `withCredentials: true`, configured CORS with `allowCredentials`, server-side cookie clear on logout.
