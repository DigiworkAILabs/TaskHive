...
import '../../widgets/task_approval_panel.dart';
...
class AdminTaskDetailScreen extends ConsumerWidget {
      ...
      body: taskAsync.when(
        data: (task) => ListView(
          ...
          children: [
            ...
            // P1.2 — Admin Approval Panel
            TaskApprovalPanel(
              task: task,
              onActionSuccess: () => ref.invalidate(taskDetailProvider(taskId)),
            ),
  ...
  Future<void> _changeStatus(
    ...
    if (newStatus == TaskStatus.cancelled) {
      final task = ref.read(taskDetailProvider(id)).valueOrNull;
      reason = await showDialog<String>(
        context: context,
        builder: (context) =>
            CancelReasonDialog(taskTitle: task?.title ?? 'this task'),
      );
